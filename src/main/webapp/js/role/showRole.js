layui.use(['layer','table','form','laydate'], function() {
    ...
    function loadVip(queryJsonVip) {
        table.render({
            ...
            , height: 412  //容器高度
            , url: '/roles/showPageByPramas' //数据接口或者访问服务器端的数据路径
            , limit: 3   //自定义每一页的数据条数
            , limits: [2, 3, 5, 8, 10]
            , even: true  //逐行背景色深浅不一
            , page: true //开启分页
            , cols: [[ //表头  field: 'id'表示从实体对象的属性中取到数据放入容器里
                ...
                , {field: 'roleName', title: '角色名', align: 'center', width: 220, sort: true}
                , {field: 'fristAuthority', title: '角色权限', width: 500, align: 'center'}
                , {field: 'createDate', title: '创建时间', width: 200, align: 'center', sort: true}
                , {field: 'status', title: '是否可用', width: 240, align: 'center', templet: '#statusTpl'}
    ...
    table.on('tool(test)',function (obj) {
        ...
        if(event=='query'){
            ...
            loadAuthority('/roles/showAllAuthorityByid?roleId='+data.id);
        }

    })
    function loadAuthority(dataUrl) {
        var setting = {
            data : {
                simpleData : {
